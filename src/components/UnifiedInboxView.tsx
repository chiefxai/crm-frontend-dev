...
import { MessageCircle, Camera as IgIcon, Send, Settings, Bot, User, Loader2, Sparkles } from 'lucide-react';
...
function ChannelSettingsModal({ channels, onClose, onSaved }: { channels: Channel[]; onClose: () => void; onSaved: () => void }) {
  ...
  const saveWhatsapp = async () => {
    ...
    setSaving('whatsapp'); setError('');
    const res = await apiFetch('/api/channels/whatsapp', { method: 'POST', body: JSON.stringify({ phoneNumberId: waPhoneId, accessToken: waToken, aiAutoReply: waAutoReply }) });
    setSaving(null);
    if (res.ok) onSaved(); else setError((await res.json()).error || 'Failed to save WhatsApp channel');
  };

  const saveInstagram = async () => {
    if (!igAccountId || !igToken) return;
    setSaving('instagram'); setError('');
    const res = await apiFetch('/api/channels/instagram', { method: 'POST', body: JSON.stringify({ igBusinessAccountId: igAccountId, accessToken: igToken, aiAutoReply: igAutoReply }) });
    setSaving(null);
    if (res.ok) onSaved(); else setError((await res.json()).error || 'Failed to save Instagram channel');
  };

  return (
    <Modal open onClose={onClose} title="Connect Channels">
      <div className="space-y-6">
        ...
        <div className="border border-slate-200 rounded-xl p-4">
          ...
          <label className="flex items-center gap-2 text-xs text-slate-500 mb-3"><input type="checkbox" checked={waAutoReply} onChange={e => setWaAutoReply(e.target.checked)} /> Let AI auto-reply to incoming messages</label>
          <button onClick={saveWhatsapp} disabled={saving === 'whatsapp'} className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white text-sm font-medium py-2 rounded-lg cursor-pointer">
        ...
        <div className="border border-slate-200 rounded-xl p-4">
          ...
          <label className="flex items-center gap-2 text-xs text-slate-500 mb-3"><input type="checkbox" checked={igAutoReply} onChange={e => setIgAutoReply(e.target.checked)} /> Let AI auto-reply to incoming DMs</label>
          <button onClick={saveInstagram} disabled={saving === 'instagram'} className="w-full bg-pink-600 hover:bg-pink-500 disabled:opacity-60 text-white text-sm font-medium py-2 rounded-lg cursor-pointer">
...
export default function UnifiedInboxView() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    const res = await apiFetch(`/api/conversations/${selectedId}/messages`, { method: 'POST', body: JSON.stringify({ text: replyText }) });
    setSending(false);
    if (res.ok) { const sent = await res.json(); setMessages(prev => [...prev, sent]); setReplyText(''); }
    else alert((await res.json()).error || 'Failed to send message');
  };
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzing(false);
    if (res.ok) { const result = await res.json(); setConversations(prev => prev.map(c => c.id === selectedId ? { ...c, ...result } : c)); }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden px-8 py-5 gap-4 min-h-0">
        {/* Conversation list */}
          ...
          {conversations.length === 0
            ...
            : conversations.map(c => (
              <button key={c.id} onClick={() => setSelectedId(c.id)}
                className={`w-full text-left px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors flex items-center gap-3 cursor-pointer ${selectedId === c.id ? 'bg-blue-50' : ''}`}>
              ...
              <>
                <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={handleAnalyze} disabled={analyzing}
                    className="ml-auto flex items-center gap-1.5 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 disabled:opacity-60 px-3 py-1.5 rounded-lg transition-colors cursor-pointer">
                    {analyzing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />} AI Summary
                  </button>
                </div>
                {selectedConversation.summary && (
                  <div className="px-5 py-3 bg-indigo-50/50 border-b border-indigo-100 text-xs space-y-1 shrink-0">
                ...
                <div className="p-4 border-t border-slate-100 flex gap-2 shrink-0">
                  <input value={replyText} onChange={e => setReplyText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
                  ...
                  <button onClick={handleSend} disabled={sending} className="bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white p-2.5 rounded-xl cursor-pointer">
