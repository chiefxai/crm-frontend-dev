...
import { X, Download, Loader2, Phone, PhoneIncoming, PhoneOutgoing, TrendingUp, Clock, DollarSign } from 'lucide-react';
import { CallLog } from '../types';
import { callCostInr, formatInr } from '../lib/pricing';
import { downloadPDF, downloadCSV } from '../shared/lib/exporters';
...
function CallRow({ c, cpm }: { c: CallLog; cpm: number }) {
  return (
    <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
      ...
      <td style={{ padding: '8px 10px' }}>
        <span style={{ fontSize: 10, fontWeight: 700, color: intentBadge(c.intent), background: intentBadge(c.intent) + '18', padding: '2px 8px', borderRadius: 100 }}>
          {c.intent}
        </span>
      </td>
...
export default function PrintableReport({
  ...
  const handlePDF = async () => {
    setPdfPct(0);
    try {
      await downloadPDF(REPORT_ID, `${orgName.replace(/\s+/g, '_')}_call_report_${fromDate}_${toDate}.pdf`, setPdfPct);
    } finally {
      setPdfPct(null);
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 100, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '24px 16px', overflowY: 'auto' }}>
      {/* Controls — not part of the PDF capture */}
      <div style={{ position: 'sticky', top: 0, zIndex: 10, width: '100%', maxWidth: 940, marginBottom: 12 }}>
        <div style={{ background: '#1e293b', borderRadius: 14, padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, boxShadow: '0 4px 24px rgba(0,0,0,0.3)' }}>
          <span style={{ color: '#f1f5f9', fontWeight: 700, fontSize: 14 }}>Report Preview</span>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <button onClick={handleCSV} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#334155', border: 'none', color: '#94a3b8', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              <Download size={13} /> Export CSV
            </button>
            <button
              onClick={handlePDF}
              disabled={pdfPct !== null}
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#2563eb', border: 'none', color: '#fff', borderRadius: 8, padding: '7px 16px', fontSize: 12, fontWeight: 600, cursor: pdfPct !== null ? 'wait' : 'pointer', opacity: pdfPct !== null ? 0.75 : 1 }}
            >
              {pdfPct !== null ? <><Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} /> Generating {pdfPct}%</> : <><Download size={13} /> Download PDF</>}
            </button>
      ...
      <div id={REPORT_ID} style={{ background: '#ffffff', width: '100%', maxWidth: 940, borderRadius: 16, overflow: 'hidden', fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif", boxShadow: '0 8px 48px rgba(0,0,0,0.18)' }}>
                      ...
                      <tbody>
                        {rows.map(r => (
                          <tr key={r.lid} style={{ borderBottom: '1px solid #f8fafc' }}>
                            ...
                            <td style={{ padding: '7px 8px' }}><span style={{ fontSize: 10, fontWeight: 700, color: r.status === 'Completed' ? '#059669' : '#94a3b8', background: r.status === 'Completed' ? '#f0fdf4' : '#f8fafc', padding: '2px 7px', borderRadius: 100 }}>{r.status || 'Pending'}</span></td>
                            <td style={{ padding: '7px 8px', color: '#475569' }}>{fmt(r.duration || 0)}</td>
                            <td style={{ padding: '7px 8px' }}><span style={{ fontSize: 10, fontWeight: 700, color: sentimentColor(r.sentiment || 'Unknown') }}>{r.sentiment || '—'}</span></td>
                            <td style={{ padding: '7px 8px' }}><span style={{ fontSize: 10, fontWeight: 700, color: intentBadge(r.intent || 'Unknown') }}>{r.intent || '—'}</span></td>
